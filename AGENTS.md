...
- Managed agent platforms are deployment surfaces, not Ercan OS constitutions. Load `MANAGED_AGENT_DEPLOYMENT.md` + `managed-agent-deployment` when recurring business agents need client/team handoff, approvals, isolated managed execution or operator-friendly live run visibility. Current connector permissions, AUP, pricing and data-processing terms are reverified at runtime; bulk unsolicited outreach is never routed through a managed provider whose terms prohibit it.
