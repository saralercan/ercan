...
- JavaScript/TypeScript: for material JS/TS architecture, runtime bugs, async/concurrency, memory leaks, browser/DOM performance, bundle size, testing, Node/Bun/Deno compatibility, dependency/security or JavaScript-focused review, load `.agents/skills/javascript-specialist-capability-pack/SKILL.md` + `docs/standards/JAVASCRIPT_SPECIALIST_ENGINE.md`. User-facing `@JavaScript` is a JIT alias that resolves to the smallest sufficient JS/TS capability pod mapped onto existing stable owners; it is not a new stable identity.
