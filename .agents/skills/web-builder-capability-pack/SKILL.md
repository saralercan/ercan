...
- ContentSiteBuilder -> `@WebArchitecture + @FrontendSystem`; load `.agents/skills/editorial-writing-capability-pack/SKILL.md` when page/blog/content creation or editorial QA is material. Astro is a JIT option for content/editorial/marketing sites, not a global default.
- WebsiteContent -> `@BrandBehavior + @TechnicalSEO + @AccessibilityQA` with the active platform owner; route copywriting, blog, UX microcopy, structured content, fact/citation review and proofreading through the Editorial & Writing Capability Pack.
...
Existing canonical QA engines such as Playwright, Lighthouse, axe-core, Storybook, shadcn/ui, SiteOne, Theme Tools and screenshot-to-code continue to be selected from existing Ercan OS standards. Material JavaScript/TypeScript diagnosis may additionally load the JavaScript Specialist Capability Pack and Chrome DevTools MCP JIT for live runtime/performance/memory evidence. Website lifecycle work may additionally use GoogleChrome/modern-web-guidance, ChromeDevTools/chrome-devtools-mcp, aidenybai/react-grab and SandeepBaskaran/design-mode through `website-lifecycle-agent-pack`.
